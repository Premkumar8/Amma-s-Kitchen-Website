...
# Database Config
# -------------------------------
app.config['SQLALCHEMY_DATABASE_URI'] = 'mysql+pymysql://root:root@localhost/amma'
...
def get_cart_summary_for_user(user_id):
    ...
    for o in orders:
        p = o.product
        qty = int(round(o.total / p.price)) if p and p.price>0 else 1
        subtotal = o.total
        items.append({'order_id': o.id, 'id': p.id, 'name': p.name, 'qty': qty, 'price': p.price, 'subtotal': subtotal, 'image': (p.images[0].image_url if p.images else None)})
        total += subtotal
        count += qty
...
def build_cart_context(user_id):
    ...
    for o in orders:
        p = o.product
        qty = 1  # if you don't have a quantity field, treat each order row as qty 1
        price = o.total  # if total stored per-order; adjust if you store per-item price separately
        subtotal = price
        total += subtotal
        count += qty
...
def index():
    ...
    return render_template('index.html', products=products, cart_items=cart_items, cart_total=cart_total, cart_count=cart_count)
